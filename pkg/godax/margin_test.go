...
func TestClient_GetMarginStatus(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "when a succesful call is made to get your margin status",
			fields: defaultFields(),
			want: MarginStatus{
				Tier:     0,
				Enabled:  true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.GetMarginStatus()
			if (err != nil) != tt.wantErr {
				t.Errorf("Client.GetMarginStatus() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Client.GetMarginStatus() = %v, want %v", got, tt.want)
			}
		})
	}
}
